...
function readme(photoCount, keyframeCount, missing, appVersion) {
  return [
    ...
    'To diagnose the vertical guidance — the dot climbing a tall obstruction and stopping short — read scan-coverage.json alongside the boundary traces in keyframes.json. The traces say what the skyline ACTUALLY was; the obstructionTop / requiredElevation / satisfiedElevation / beyondTilt arrays say what the app believed and asked for. obstructionTop is only ever a LOWER bound refined from frames whose skyline ran off the top edge, so where a frame stopped clipping it stopped improving, and the guidance trail entries carry requiredElevationDeg, wantsLift, liftDeg and clippedFraction at each sampled moment to show where the climb ended and why.',
